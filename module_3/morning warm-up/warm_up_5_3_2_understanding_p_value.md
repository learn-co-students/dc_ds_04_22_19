...
### Understanding P-values a bit 
1. let's start by thinking about a game where you need to identify 
20 cards drawn randomly with replacement from a 52 card deck.
...
5. Use a while loop and inside it a for loop.

	-    stop the while loop only when you have 5 rounds with 10 correct guesses or more - keep the loop running for 20 steps always (Don't use break!)
	-    Run the for loop for a range of 20 (our test for identification)
	-    Each run draw randomly from your deck a card
	-    Also choose randomly from the list of 4 elements (spades, diamond etc..)
